...
class DocumentLayoutInfoTest {
    ...
    fun `side-specific formats are separate from global`() {
        ...
        val left = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT), pageWidth = WIDTH_A4)
        val layout = DocumentLayoutInfo(pageFormats = listOf(global, left))
        val result = layout.getPageFormatsWithDefault(null)

        assertEquals(2, result.size)

        val globalResult = result.first { it.selector == null }
        assertEquals(WIDTH_A4, globalResult.pageWidth)
        assertEquals(HEIGHT_A4, globalResult.pageHeight)

        val leftResult = result.first { it.selector?.side == PageSide.LEFT }
    ...
    fun `side-specific formats with same side are merged`() {
        val leftMargin = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT), pageWidth = WIDTH_A4)
        val leftColumns = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT), columnCount = 3)
        val layout = DocumentLayoutInfo(pageFormats = listOf(leftMargin, leftColumns))
        val result = layout.getPageFormatsWithDefault(null)

        assertEquals(1, result.size)
        assertEquals(PageSide.LEFT, result.single().selector?.side)
        assertEquals(WIDTH_A4, result.single().pageWidth)
    ...
    fun `default, global, and side-specific formats coexist`() {
        ...
        val left = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT), columnCount = 2)
        val right = PageFormatInfo(selector = PageFormatSelector(side = PageSide.RIGHT), columnCount = 3)
        ...
        val rightResult = result.first { it.selector?.side == PageSide.RIGHT }
        assertEquals(3, rightResult.columnCount)
    }

    @Test
    fun `range-only format stays separate from global`() {
        val global = PageFormatInfo(pageWidth = WIDTH_A4)
        val ranged = PageFormatInfo(selector = PageFormatSelector(range = Range(2, 5)), columnCount = 2)
        val layout = DocumentLayoutInfo(pageFormats = listOf(global, ranged))
        val result = layout.getPageFormatsWithDefault(null)
        ...
        val rangedResult = result.first { it.selector?.range != null }
        assertEquals(2, rangedResult.columnCount)
    }

    @Test
    fun `formats with same selector merge`() {
        val first = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT, range = Range(1, 3)), pageWidth = WIDTH_A4)
        val second = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT, range = Range(1, 3)), columnCount = 2)
        val layout = DocumentLayoutInfo(pageFormats = listOf(first, second))
        val result = layout.getPageFormatsWithDefault(null)
    ...
    @Test
    fun `formats with same side but different range stay separate`() {
        val first = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT, range = Range(1, 3)), pageWidth = WIDTH_A4)
        val second = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT, range = Range(4, 6)), columnCount = 2)
        val layout = DocumentLayoutInfo(pageFormats = listOf(first, second))
        val result = layout.getPageFormatsWithDefault(null)
    ...
    @Test
    fun `default, global, side, and range formats coexist`() {
        val userGlobal = PageFormatInfo(pageHeight = HEIGHT_SMALL)
        val left = PageFormatInfo(selector = PageFormatSelector(side = PageSide.LEFT), columnCount = 2)
        val ranged = PageFormatInfo(selector = PageFormatSelector(range = Range(1, 3)), columnCount = 3)
        val layout = DocumentLayoutInfo(pageFormats = listOf(userGlobal, left, ranged))
        val result = layout.getPageFormatsWithDefault(DEFAULT_FORMAT)
        ...
        val rangedResult = result.first { it.selector?.range != null }
        assertEquals(3, rangedResult.columnCount)
    }
