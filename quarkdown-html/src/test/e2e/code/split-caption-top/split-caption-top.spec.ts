...
test("shows a top caption once, on the first portion with content of a split code block", async (page) => {
    ...
    // A top caption belongs to the first portion of the code block that displays content,
    // never standing alone on a page.
    await expect(pages.nth(1).locator("figure > figcaption")).toHaveCount(1);
    await expect(pages.nth(0).locator("figure")).toHaveCount(0);
});
