...
test("preserves indentation across split", async (page) => {
    ...
    // Lines 3-4: 8, 12 spaces
    expect(await secondPageLines.nth(0).textContent()).toMatch(/^ {8}Line 3/);
    expect(await secondPageLines.nth(1).textContent()).toMatch(/^ {12}Line 4/);
});
